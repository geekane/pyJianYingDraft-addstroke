...
from .text_segment import Text_segment, Text_style, Text_border  # 引入Text_segment 和 Text_style
...
class Video_segment(Media_segment):
    ...
    def add_text(self, text: str, time_range: Timerange, *, text_style: Optional[Text_style] = None, border: Optional[Text_border] = None, clip_settings: Optional[Clip_settings] = None) -> Text_segment:
       """为视频片段添加文本

       Args:
            text (`str`): 文本内容
            time_range (`Timerange`): 文本显示的时间范围
            text_style (`Text_style`, optional): 文本样式
            border (`Text_border`, optional): 文字描边设置，默认为None
            clip_settings (`Clip_settings`, optional): 图像调节设置, 默认不作任何变换
       """
       text_seg = Text_segment(text, time_range, style = text_style, clip_settings=clip_settings, border=border)
       self.extra_material_refs.append(text_seg.material_id) # 将text_seg的material_id添加进extra_material_refs
       return text_seg # 返回一个Text_segment对象
